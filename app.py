...
if pedido.strip():
    ...
    if dados_corrigidos["subcontracao"] or dados_corrigidos["subcontratacoes_detectadas"]:
        st.error("Preencher subcontratação externa.")
        if dados_corrigidos["subcontracao"]:
            st.write(f"- Subcontratação indicada: {dados_corrigidos['subcontracao']}")
        if dados_corrigidos["subcontratacoes_detectadas"]:
